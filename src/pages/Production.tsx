...
export default function Production() {
  ...
  const totalBreakdownKg = Object.entries(breakdown).reduce((acc: number, [_, weight]) => {
    return acc + (weight as number);
  }, 0);

  const handleSave = () => {
    if (!eggCount || !feedConsumed) {
      Swal.fire({
        title: 'Data Tidak Lengkap',
        text: 'Mohon isi total produksi dan pakan konsumsi.',
        icon: 'warning',
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        // Reset or redirect here in real app
      }
    });
  };
    ...
    <div className="max-w-4xl mx-auto space-y-8 pb-20">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-[10px] md:text-sm mt-1 uppercase font-bold tracking-widest opacity-70">Pencatatan hasil harian & operasional.</p>
        </div>
        <div className="bg-white border border-slate-200 px-4 py-2 shadow-sm flex items-center space-x-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="md:col-span-1 space-y-6">
          <div className="bg-white p-6 border border-slate-200 shadow-sm space-y-4">
            ...
            <div>
              <label className="text-[10px] font-bold uppercase tracking-widest text-slate-400 block mb-2">Telur Dibuang/Afkir (Papan/Kg)</label>
              <input 
                type="number" 
                placeholder="0"
                value={discardedEggs || ''}
          ...
          <div className="bg-slate-900 text-white p-6 shadow-xl border border-slate-800">
            <div className="flex items-center space-x-2 text-amber-500 mb-2">
                <Info size={14} />
                <span className="text-[10px] font-bold uppercase tracking-widest">Tips Pengisian</span>
            </div>
            <p className="text-xs leading-relaxed text-slate-400">Pastikan timbangan pakan menggunakan satuan **kilogram**. Akurasi data sangat menentukan margin laba bersih harian.</p>
          </div>
        </div>
          ...
          <div className="bg-white p-8 border border-slate-200 shadow-sm relative overflow-hidden">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
              <div>
                ...
                <p className="text-[9px] md:text-[10px] text-emerald-600 font-bold uppercase tracking-widest mt-1">Terdistribusi Harian</p>
              </div>
              <div className="px-3 py-1 bg-slate-100 rounded-sm text-[9px] md:text-[10px] font-bold text-slate-600 border border-slate-200 self-start sm:self-center">TOTAL: {totalBreakdownKg} KG</div>
            </div>
              ...
              {Object.values(EggCategory).map((cat) => (
                <div key={cat} className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    />
                    <span className="absolute right-4 top-1/2 -translate-y-1/2 text-[10px] font-bold text-slate-400 uppercase">kg/papan</span>
                  </div>
                </div>
              ))}
            </div>
