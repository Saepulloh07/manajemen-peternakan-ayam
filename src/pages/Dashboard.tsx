...
import { formatCurrency, calculateHDP, cn } from '../lib/utils';
...
function MetricCard({ label, value, subValue, icon: Icon, trend, trendValue, color = "bg-white", progress }: MetricCardProps & { progress?: number }) {
  return (
    <div className={cn("p-4 md:p-5 border border-slate-200 shadow-sm transition-all hover:shadow-md relative overflow-hidden bg-white", color)}>
...
export default function Dashboard() {
  ...
  const targetHDP = 95;
  const isCullingRecommended = false;
  ...
  return (
    <div className="grid grid-cols-12 gap-6 pb-12">
      ...
      <div className="col-span-12 md:col-span-3">
        <MetricCard 
          label="Omset Bulanan" 
          value={formatCurrency(142500000)} 
          icon={TrendingUp}
          trend="up"
          trendValue="+12.4% vs Bulan Lalu"
        />
      </div>
      <div className="col-span-12 md:col-span-3">
        <MetricCard 
          ...
          value={`${currentHDP}%`} 
          progress={currentHDP}
          subValue={`Target Strain: 90.0% - 95.0%`}
          icon={Activity}
        />
      </div>
      <div className="col-span-12 md:col-span-3">
        <MetricCard 
          ...
          value="0.42%" 
          subValue="8 Ekor Mati (Minggu Ini)"
          icon={Skull}
          trend="down"
          trendValue="Sehat"
        />
      ...
      <div className="col-span-12 md:col-span-3">
        <MetricCard 
          label="Estimasi Stok Pakan" 
          value="1,250 kg" 
          subValue="Sisa 4 Hari (Payakumbuh)"
          icon={Package}
          trend="neutral"
          trendValue="Lead: 3-5d"
        />
      </div>
        ...
        <div className="bg-white border border-slate-200">
          <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
            <div>
              ...
              <p className="text-[10px] text-slate-400 font-medium uppercase tracking-wider">Statistik 7 Hari Terakhir</p>
            </div>
        ...
        <div className="bg-slate-900 text-white shadow-xl p-6 relative overflow-hidden border border-slate-800">
          ...
          <div className="space-y-6">
            <div className="flex gap-3 items-start">
              ...
              <div>
                <p className="text-sm font-bold">Peringatan Stok Pakan</p>
                <p className="text-xs text-slate-400 mt-1 leading-relaxed">Jagung habis dalam 48 jam. Order Payakumbuh diperlukan hari ini untuk antisipasi lead time.</p>
              </div>
            </div>
            ...
            <div className="flex gap-3 items-start border-t border-slate-800 pt-6">
              ...
              <div>
                <p className="text-sm font-bold">Analisis Afkir Kandang</p>
                <p className="text-xs text-slate-400 mt-1 leading-relaxed">Margin operasional &lt; 10% selama 3 hari berturut-turut. Rekomendasi: Penjualan Afkir.</p>
              </div>
            </div>

            <button className="w-full py-3 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-[10px] font-bold uppercase tracking-widest transition-colors mt-2 text-amber-400">
              KIRIM ALERT KE WHATSAPP OWNER
            </button>
          </div>
        </div>
